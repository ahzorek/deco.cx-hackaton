...
export default function AskUserPreference(
  ...
  //pessima pratica
  const elId = `askUser_${cookieName}`;
  return (
    <div id={elId} class="p-12 max-w-6xl mx-auto">
      ...
      <section class="grid md:grid-cols-2 gap-6 text-gray-50">
        {options.map((option) => {
          ...
          return (
            <CookieSetterButton
              cookieData={{ value, cookieName, cookieExpires, wrapper: elId }}
              class={`flex text-left flex-col justify-start items-start gap-4 p-8 bg-${bgColor} rounded-md shadow-lg transition hover:scale-105`}
            >
